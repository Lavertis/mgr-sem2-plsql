...
DECLARE
    ...
    i number := 1;
BEGIN
    FOR rekord IN (SELECT s.ID_STUDENT, s.imie, s.nazwisko, count(e.ID_EGZAMIN) as liczba_egzaminow, sum(e.PUNKTY) as liczba_punktow
                    FROM Studenci s
    ...
    LOOP
        ...
        nt_student_tab(i) := NT_Student_Data(rekord.ID_STUDENT, rekord.imie, rekord.nazwisko, rekord.liczba_egzaminow, rekord.liczba_punktow);
        i := i + 1;
    END LOOP;

    FOR i IN 1..nt_student_tab.count LOOP
